...
DELIMITER ;
DELIMITER ;;
CREATE DEFINER=`michelek`@`127.0.0.1` FUNCTION `func_person_born`(
	`in_persoon` INT
) RETURNS char(10) CHARSET utf8 COLLATE utf8_estonian_ci
func_label:BEGIN
...
DELIMITER ;
DELIMITER ;;
CREATE DEFINER=`michelek`@`127.0.0.1` FUNCTION `func_person_dead`(
	`in_persoon` CHAR(10)
) RETURNS char(10) CHARSET utf8 COLLATE utf8_estonian_ci
func_label:BEGIN
    ...
    DECLARE date_cursor CURSOR FOR
        ...
        WHERE k0.persoon = in_persoon
        AND not a.nonPerson
        ORDER BY a.prioriteetSurm desc, CHAR_LENGTH(k0.Surm) DESC;
